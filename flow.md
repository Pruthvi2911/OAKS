...
- **Step 9: Indian Number Plates & Smart Load Suggestion** — Completed. Driver page uses a 4-field structured Indian plate format. Master page features a "Best Next Load" smart banner utilizing `suggestOptimalNextLoad()` to compute the optimal vehicle-bay pairing across the entire queue, eliminating trial and error.
- **UI Restructuring** — Completed. Vehicle cards across Deck, Queue, and Recommendation panels converted to strict vertical `flex-col` layouts for better text wrapping, label clarity, and full number plate display without truncation.
