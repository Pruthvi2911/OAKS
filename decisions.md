...
| **Language** | JavaScript (ES6+, ES Modules) | Explicit requirement: React + JavaScript (No TypeScript). `"type": "module"` set in `package.json`. |
...
7. **Offline Cast Off Protection**: Cast Off is strictly disabled while offline or working with stale data to avoid departed state mismatch.
8. **Multi-Master Lock**: One active master per crossing (`MultiMasterGuard.js`). Secondary devices are locked into `VIEW ONLY` mode with an option to claim control if authorized.
